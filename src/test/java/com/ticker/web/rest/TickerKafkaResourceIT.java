...
import com.ticker.config.KafkaProperties;
...
class TickerKafkaResourceIT {
    ...
    void setup() {
        ...
        TickerKafkaResource kafkaResource = new TickerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ticker-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ticker-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
